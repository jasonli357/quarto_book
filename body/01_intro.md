...
AI Agent（智能体）能够显著提升科研效率，实现多步骤任务的自动化执行。Claude Code 是 Anthropic 推出的一款命令行 AI 编程助手（CLI），能够直接在终端中理解项目上下文、执行代码、读写文件。对于经管类研究者而言，将 Claude Code 与 Stata 结合，可以实现：
...
即使将 Stata 添加到系统环境变量中，Claude Code 有时仍然无法正确定位 Stata 的安装路径。因为 Claude Code 可能用 “精确搜索” 命令查找名为 “stata.exe” 的可执行文件，但正确的可执行文件名是 “StataMP-64.exe”。有时候 Claude Code 也会用 “模糊搜索” 命令，这时就能找到环境变量中的 Stata。
...
| **前提条件** | 无 | 需要 Stata 授权文件（`stata.lic`） |

接下来的两小节将分别详细介绍这两种方案的配置方法。
