...
![w:350](https://fig-lianxh.oss-cn-shenzhen.aliyuncs.com/20260407205148.png)
...
Shift+Tab 选择模式：普通模式 / plan mode / accept edits on
- plan mode：当你向 Claude 发出指令时，Claude 不会直接开始编写代码或执行任何实际的文件修改，而是先对任务进行分析，并输出一份详细的执行计划清单。
- accept edits on：自动修改和保存代码文件。但当它需要执行终端命令（如 `npm install`、`mkdir`、`git push`）、读取环境变量或发起网络请求时，依然会停下来征求你同意。
...
#### Step 3：确认计划没问题后，开始执行（用时约 10 min）
...
3. 尤其关注缺失值的情况。例如：合并数据集后，会不会某一个数据大量缺失导致合并数据集大量缺失；计算某个指标时，
